...
describe('GeminiService', () => {
  ...
  describe('cosineSimilarity', () => {
    ...
    it('should calculate correct similarity for arbitrary vectors', () => {
      const vecA = [1, 1];
      const vecB = [2, 2];
      // They point in the same direction, so similarity is 1
      const result = service.cosineSimilarity(vecA, vecB);
      expect(result).toBeCloseTo(1);
    });
  });
  ...
  describe('analyzeCv', () => {
    it('should parse Ollama JSON response and transform holland code', async () => {
      ...
      global.fetch = jest.fn().mockResolvedValue({
        ok: true,
        json: jest.fn().mockResolvedValue({ response: JSON.stringify(mockResponse) }),
      });
      ...
      expect(result.hollandCode.label).toBe('Thinker'); // mapped from HOLLAND_PERSONALITY
      expect(result.hollandCode.distribution.length).toBe(6);
      expect(result.hollandCode.distribution.find(d => d.code === 'I')?.value).toBe(50);
    });
  });
});
